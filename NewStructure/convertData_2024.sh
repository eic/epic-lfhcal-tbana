...
if [ $1 = "fbock" ]; then 
	...
	dataRaw=/media/fbock/T7/CAEN_Sept24_TB_PS
	dataDir=/media/fbock/Samsung_T5/LFHCAL_TB/202408_PST09/CAENdata
elif [ $1 = "fbockTB" ]; then 
...
#   runs='492 505'
  for runNr in $runs; do 
    Convert -c $dataDir/Run$runNr\_list.txt -o $dataRaw/MuonRuns/rawRedStack_$runNr.root -d 1 -f -m $configdir/TB2024/mappingFile_202409_CAEN_reduced.txt -r $configdir/TB2024/DataTakingDB_202409_CAEN.csv
  done;
...
#   runs='271 277 244 250 282 283 460 456 457 454 526 527 528 554 559 552' 
  ...
  runs='271 277 244 250 282 283' 
  for runNr in $runs; do 
    ./Convert -c $dataRaw/Run$runNr\_list.txt -o $dataDir/MuonRuns/raw_$runNr.root -d 1 -f -m $configdir/TB2024/mappingFile_202409_CAEN.txt -r $configdir/TB2024/DataTakingDB_202409_CAEN.csv
  done;
  ...
  for runNr in $runs; do 
    ./Convert -c $dataRaw/Run$runNr\_list.txt -o $dataDir/ElectronRuns/raw_$runNr.root -d 1 -f -m $configdir/TB2024/mappingFile_202409_CAEN.txt -r $configdir/TB2024/DataTakingDB_202409_CAEN.csv
  done;
elif [ $2 == "electronsB" ]; then 
  runs='333 334 336 337 338 ' 
  for runNr in $runs; do 
    Convert -c $dataDir/Run$runNr\_list.txt -o $dataRaw/ElectronRuns/raw_$runNr.root -d 1 -f -m $configdir/TB2024/mappingFile_202409_CAEN.txt -r $configdir/TB2024/DataTakingDB_202409_CAEN.csv
  done;
  ...
  for runNr in $runs; do 
    ./Convert -c $dataRaw/Run$runNr\_list.txt -o $dataDir/HadronRuns/raw_$runNr.root -d 1 -f -m $configdir/TB2024/mappingFile_202409_CAEN.txt -r $configdir/TB2024/DataTakingDB_202409_CAEN.csv
  done;
elif [ $2 == "hadronsB" ]; then 
  runs='340 349 346 350 357 360 362 367 368' 
  for runNr in $runs; do 
    Convert -c $dataDir/Run$runNr\_list.txt -o $dataRaw/HadronRuns/raw_$runNr.root -d 1 -f -m $configdir/TB2024/mappingFile_202409_CAEN.txt -r $configdir/TB2024/DataTakingDB_202409_CAEN.csv
  done;
